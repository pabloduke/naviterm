...
func drawBorder(x int, y int, barChart chart.BarChart) {
	...
	for ; XPos < width+x+5; XPos++ {
		DrawText(XPos, y, symbols.Hbar, terminal.ColorRed, terminal.ColorDefault)
		DrawText(XPos, y-height-1, symbols.Hbar, terminal.ColorLightCyan, terminal.ColorDefault)
	}
}

func drawTitle(x int, y int, height int, barChart chart.BarChart) {
	DrawText(x, y-height-2, barChart.Title, terminal.ColorWhite, terminal.ColorDefault)
}

func drawXLabel(x int, y int, height int, barChart chart.BarChart) {
	DrawText(x-10, y-(height/2), barChart.XLabel, terminal.ColorWhite, terminal.ColorDefault)
}
